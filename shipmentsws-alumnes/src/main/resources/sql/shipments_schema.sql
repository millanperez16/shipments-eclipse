...
DROP TABLE IF EXISTS `tracknum`;
CREATE TABLE IF NOT EXISTS `tracknum` (
...
FOR EACH ROW
BEGIN
	IF (NEW.`type` = 'RECEPTION') THEN
    	SET NEW.`tracking_number` = (SELECT track FROM tracknum);
    	UPDATE tracknum SET track = track + 1;
    END IF;
END;
//
DELIMITER ;
